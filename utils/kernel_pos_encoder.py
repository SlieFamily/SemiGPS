...
class KernelPENodeEncoder(torch.nn.Module):
    ...
    Args:
        dim_in: Expected original input node features dim
        dim_pe: Size of the kernel-based PE embedding
        dim_emb: Size of final node embedding
        model_type: Encoder NN model type for PEs (`mlp` or `linear`)
        n_layers: Num. layers in PE encoder model
        norm_type: Raw PE normalization layer type (`BatchNorm` or `None`)
        ksteps: List of k-steps for which to compute kernel-based statistics
        pass_as_var: True if Pass PE also as a separate variable
