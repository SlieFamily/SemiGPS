...
def compute_posenc_stats(data, pe_types, is_undirected, max_freqs=10, eigvec_norm='L2', **kargs):
    ...
    Args:
        ...
        (Optional args for given pe_types)
        laplacian_norm_type: The Laplacian normalization scheme for the graph to 'LapPE', 'EquivStableLapPE', 'SignNet' and 'HKdiagSE'
        rw_ksteps: List of k-steps for which to compute 'RWSE'
        hk_diag_ksteps: List of k-steps for which to compute 'HKdiagSE'
        num_spatial_types: The distance up to which types are calculated for 'Graphormer'
    ...
    # Laplacian norm setting.
    if 'laplacian_norm' in kargs:
        laplacian_norm_type = kargs['laplacian_norm']  # sym or none
    ...
    # Random Walks.
    if 'RWSE' in pe_types:
        if 'rw_ksteps' in kargs:
            rw_kernel_times = kargs['rw_ksteps']
        if len(rw_kernel_times) == 0:
    ...
    if 'GraphormerBias' in pe_types:
        if 'num_spatial_types' in kargs:
            data = graphormer_pre_processing(
                data,
                kargs['num_spatial_types'],
            )
